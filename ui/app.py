...
def _render_stats(model: str):
    try:
        if "Hybrid" in model:
            ...
            st.info(
                f"📊 **VSM Stats**\n\n"
                f"🔤 Terms: {svc.vectorizer.vocabulary_.__len__():,}\n"
...
def _show_matching(result: dict, query: str, model: str):
    try:
        if "BERT" in model and "Hybrid" not in model:
            ...
            for token in tokens:
                ...
                if d["tfidf"] > 0:
                    st.write(
                        f"**`{token}`:** "
                        f"TF={d['tf']:.4f} | IDF={d['idf']:.4f} | TF-IDF={d['tfidf']:.4f}"
                    )
                else:
            ...
            for token in tokens:
                ...
                if term_score > 0:
                    st.write(
                        f"**`{token}`:** "
                        f"TF={tf} | BM25 contribution={term_score:.4f}"
                    )
                else:
                    st.write(f"**`{token}`:** not found in document")
        ...
        st.write(f"Matching details error: {e}")
